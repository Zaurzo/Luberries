...
Some functions in this library utilize the `debug` library. Luberries respects the decision to strip or modify the debug library, so some features may be limited or turned into no-ops if you do this. I could avoid this entirely by making a C module, but I wish to keep the library purely Lua.
